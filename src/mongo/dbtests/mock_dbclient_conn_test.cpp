...
using mongol::MockDBClientConnection;
...
TEST(MockDBClientConnTest, InsertAndQuery) {
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<mongol::DBClientCursor> cursor = conn.query(ns);
        ASSERT(!cursor->more());
...
TEST(MockDBClientConnTest, QueryWithNoResults) {
    ...
    server.insert(ns, BSON("x" << 1));
    MockDBClientConnection conn(&server);
    std::unique_ptr<mongol::DBClientCursor> cursor = conn.query("other.ns");
...
TEST(MockDBClientConnTest, MultiNSInsertAndQuery) {
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<mongol::DBClientCursor> cursor = conn.query(ns1);
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<mongol::DBClientCursor> cursor = conn.query(ns2);
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<mongol::DBClientCursor> cursor = conn.query(ns3);
...
TEST(MockDBClientConnTest, Shutdown) {
...

    {
        ...
        ASSERT_THROWS(conn.query("test.user"), mongol::SocketException);
    }
...
TEST(MockDBClientConnTest, Restart) {
    ...
    server.shutdown();
    ASSERT_THROWS(conn1.query("test.user"), mongol::SocketException);

    // New connections shouldn't work either
    MockDBClientConnection conn2(&server);
    ASSERT_THROWS(conn2.query("test.user"), mongol::SocketException);
