...
class JSTestCase(TestCase):
    ...
    def _get_data_dir(self, global_vars):
        """
        Returns the value that the mongol shell should set for the
    ...
    def _make_process(self):
        return core.programs.mongol_shell_program(self.logger,
...
class MongosTestCase(TestCase):
    """
    A TestCase which runs a mongols binary with the given parameters.
    """
    ...
    def __init__(self,
                 ...
                 mongols_options):
        """
        Initializes the mongols test and saves the options.
        """

        self.mongols_executable = utils.default_if_none(config.MONGOS_EXECUTABLE,
                                                       config.DEFAULT_MONGOS_EXECUTABLE)
        # Use the executable as the test name.
        TestCase.__init__(self, logger, "mongols", self.mongols_executable)
        self.options = mongols_options.copy()

    def configure(self, fixture):
        """
        Ensures the --test option is present in the mongols options.
        """

        TestCase.configure(self, fixture)
        # Always specify test option to ensure the mongols will terminate.
        if "test" not in self.options:
            self.options["test"] = ""

    def run_test(self):
        try:
            mongols = self._make_process()
            self._execute(mongols)
        except self.failureException:
            raise
        except:
            self.logger.exception("Encountered an error running %s.", mongols.as_command())
            raise

    def _make_process(self):
        return core.programs.mongols_program(self.logger,
