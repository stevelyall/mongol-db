...
class Suite(object):
    ...
    TESTS_ORDER = ("cpp_unit_test", "cpp_integration_test", "db_test", "js_test", "mongols_test")
    ...
    def _get_tests_for_group(self, test_kind):
        ...
        # The mongols_test doesn't have to filter anything, the test_info is just the arguments to
        # the mongols program to be used as the test case.
        if test_kind == "mongols_test":
            mongols_options = test_info  # Just for easier reading.
            if not isinstance(mongols_options, dict):
                raise TypeError("Expected dictionary of arguments to mongols")
            return [mongols_options]
