...
def shouldKill( c, root=None ):
    ...
    # if root directory is provided, see if command line matches mongold process running
    # with the same data directory

    if root and re.compile("(\W|^)mongold(.exe)?\s+.*--dbpath(\s+|=)%s(\s+|$)" % root).search( c ):
        return True

    if ( c.find( "buildbot" ) >= 0 or c.find( "slave" ) >= 0 ) and c.find( "/mongol/" ) >= 0:
        return True
