// Test mongols implementation of time-limited operations: verify that mongols correctly forwards max
// time to shards, and that mongols correctly times out max time sharded getmore operations (which
// are run in series on shards).
//
// Note that mongols does not time out commands or query ops (which remains responsibility of mongold,
// pending development of an interrupt framework for mongols).
...
// Helper function to configure "maxTimeAlwaysTimeOut" fail point on shards, which forces mongold to
...
// Helper function to configure "maxTimeAlwaysTimeOut" fail point on shards, which prohibits mongold
...
//
// Test that mongols correctly forwards max time to shards for sharded queries.  Uses
// maxTimeAlwaysTimeOut to ensure mongold throws if it receives a max time.
//
...
assert.throws(function() { cursor.next() },
              [],
              "expected query to fail in mongold due to maxTimeAlwaysTimeOut fail point");
...
assert.doesNotThrow(function() { cursor.next() },
                    [],
                    "expected query to not hit time limit in mongold");

//
// Test that mongols correctly times out max time sharded getmore operations.  Uses
// maxTimeNeverTimeOut to ensure mongold doesn't enforce a time limit.
//
...
assert.doesNotThrow(function() { cursor.next(); },
                    [],
                    "did not expect mongols to time out first batch of query");
...
//
// Test that mongols correctly forwards max time to shards for sharded commands.  Uses
// maxTimeAlwaysTimeOut to ensure mongold throws if it receives a max time.
//
...
assert.commandFailed(res,
                     "expected validate to fail in mongold due to maxTimeAlwaysTimeOut fail point");
assert.eq(res["code"],
          exceededTimeLimit,
...
assert.commandWorked(coll.runCommand("validate", {maxTimeMS: 60*1000}),
                     "expected validate to not hit time limit in mongold");

// Positive test for "count".
configureMaxTimeAlwaysTimeOut("alwaysOn");
res = coll.runCommand("count", {maxTimeMS: 60*1000});
assert.commandFailed(res,
                     "expected count to fail in mongold due to maxTimeAlwaysTimeOut fail point");
assert.eq(res["code"],
          exceededTimeLimit,
...
assert.commandWorked(coll.runCommand("count", {maxTimeMS: 60*1000}),
                     "expected count to not hit time limit in mongold");

// Positive test for "collStats".
configureMaxTimeAlwaysTimeOut("alwaysOn");
res = coll.runCommand("collStats", {maxTimeMS: 60*1000});
assert.commandFailed(res,
                     "expected collStats to fail in mongold due to maxTimeAlwaysTimeOut fail point");
assert.eq(res["code"],
          exceededTimeLimit,
...
assert.commandWorked(coll.runCommand("collStats", {maxTimeMS: 60*1000}),
                     "expected collStats to not hit time limit in mongold");
...
res = coll.runCommand("mapReduce", {map: function() { emit(0, 0); },
                                    ...
                                    maxTimeMS: 60*1000});
assert.commandFailed(res,
                     "expected mapReduce to fail in mongold due to maxTimeAlwaysTimeOut fail point");
assert.eq(res["code"],
          exceededTimeLimit,
...
assert.commandWorked(coll.runCommand("mapReduce", {map: function() { emit(0, 0); },
                                                   ...
                                                   out: {inline: 1},
                                                   maxTimeMS: 60*1000}),
                     "expected mapReduce to not hit time limit in mongold");
...
res = coll.runCommand("aggregate", {pipeline: [],
                                    maxTimeMS: 60*1000});
assert.commandFailed(res,
                     "expected aggregate to fail in mongold due to maxTimeAlwaysTimeOut fail point");
assert.eq(res["code"],
          exceededTimeLimit,
...
assert.commandWorked(coll.runCommand("aggregate", {pipeline: [],
                                                   maxTimeMS: 60*1000}),
                     "expected aggregate to not hit time limit in mongold");
...
assert.commandFailed(res,
                     "expected moveChunk to fail in mongold due to maxTimeAlwaysTimeOut fail point");
assert.eq(res["code"],
          exceededTimeLimit,
...
assert.commandWorked(admin.runCommand({moveChunk: coll.getFullName(),
                                       ...
                                       to: "shard0000",
                                       maxTimeMS: 1000*60*60*24}),
                     "expected moveChunk to not hit time limit in mongold");
