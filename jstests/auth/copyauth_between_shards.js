...
function copydbWithinShardedCluster(useReplSets, passCredentials, useAuth) {
    ...
    var test1 = mongols.getDB('test1');
    var test2 = mongols.getDB('test2');

    if (useAuth) {
        mongols.getDB("admin").createUser({user: "super", pwd: "super", roles: ["root"]});
        assert.throws(function() { mongols.getDB("test1")["test1"].findOne(); });
        mongols.getDB("admin").auth("super", "super");
    }
    ...
    if (passCredentials) {
        var result = mongols.getDB('admin').copyDatabase('test1', 'test2', undefined, "super", "super");
    }
    else {
        var result = mongols.getDB('admin').copyDatabase('test1', 'test2');
    }
    printjson(result);
